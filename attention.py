...
class Attention(nn.Module):
    def __init__(self, embed_dim=64, output_dim=1, device = 'cpu'):
    ...
    def forward(self, x, mask=None):
        if mask is None:
            ...
            mask = torch.where(mask == 0, torch.tensor(-1e7).to(self.device), torch.tensor(0.0).to(self.device))
            weight = torch.softmax(self._aggregate(x).squeeze(-1) + mask, dim=-1).float().unsqueeze(-1)
            weight = torch.where(torch.isnan(weight), torch.tensor(0.0).to(self.device), weight)
